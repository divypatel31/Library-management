...
exports.getStats = async (req, res) => {
  try {
    ...
    // NEW: Variable for reading history
    let totalHistory = 0; 
    ...
    if (role === 'Admin' || role === 'Librarian') {
       ...
       // 3. NEW: Total Reading History (ALL books ever issued, whether returned or not)
       const [history] = await db.query('SELECT COUNT(*) as count FROM issued_books WHERE user_id = ?', [req.user.id]);
       totalHistory = history[0].count;
    }

    // Pass totalHistory to the frontend
    return res.json({ 
      role, 
      totalUsers, 
      totalBooks, 
